...
MODELS = os.path.join(BASE_DIR, "model_data")

LIST_JSON = os.path.join(DATOS_DIR, "list.json")
QUESTIONS = os.path.join(DATOS_DIR, "preguntas.json")
...
def process(model, risk):
    ...
    df_examen = None
    modelo_correcto = nombre_archivo_valido(model) + ".json"
    ruta = os.path.join(MODELS, modelo_correcto)
    fase1 = True
...
def procesar_solicitudes():
    ...
    for i, fila in solicitudes.iterrows():
        ...
        modelo = nombre_archivo_valido(fila["Modelo"]) + ".json"
        ...
        if correcto:
            ...
            print(f"El modelo {fila['Modelo']} no ha podido ser procesado.")
